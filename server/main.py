...
key_token = AES.get_random_bytes(32)
...
class Main(Resource):
	...
	def post(self):
		...
		if token != None:
			...
			try:
				sender = jwt.decode(token, key_token, algorithms="HS256")
			...
			if update == "True":
				m_cursor.execute(f"""SELECT * FROM messages WHERE is_read = (?) AND sender_id = (?)""", (False, sender_id))
				data = m_cursor.fetchall()
